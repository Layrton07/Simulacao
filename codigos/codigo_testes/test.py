...
class ModeloPropagacaoGuerraEPandemia:
    def __init__(self, tamanhoCampoBatalha):
        ...
        self.tamanhoCampoBatalha = tamanhoCampoBatalha
    ...
    # Necessario que haja uma porcentagem inicialmente de soldados doentes (propagação do vírus)
    ...
    def propagarMortalidade(self):
        for i in range(len(self.populacao)):
            for j in range(len(self.populacao[i])):
                ...
                if self.populacao[i][j].estado == Estado.saudavel and self.populacao[i][j].vivo:
    ...
    def simulation(self, geracoes):
        for i in range(geracoes):
            ...
            # Calcula a porcentagem de mortes em relação ao número total de soldados na geração atual
            porcentagemMortes = (totalMortos / totalSoldados) * 100

            # Ajusta o tamanho da população na próxima geração inversamente proporcional à porcentagem de mortes
            taxaSobrevivencia = 1 - (porcentagemMortes / 100)
            self.tamanhoCampoBatalha = int(self.tamanhoCampoBatalha * taxaSobrevivencia)
            
            # Se o tamanho da população for menor que 1, encerre a simulação
            if self.tamanhoCampoBatalha < 1:
                print("A população foi completamente dizimada. Simulação encerrada.")
                break

            # Atualiza o tamanho da população na próxima geração
            self.populacao = []
            for i in range(self.tamanhoCampoBatalha):
                self.populacao.append([])
                for j in range(self.tamanhoCampoBatalha):
                    # Inicialmente, todos os soldados estão saudáveis
                    self.populacao[i].append(Soldado(Estado.saudavel))
